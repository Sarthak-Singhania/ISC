...
def games():
    ...
    if request.headers['admin-header'].title() == 'Yes':
        cursor.execute(
            "select `Sports_Name`,`URL`,`Enabled`,`Info` from `games` order by `Sports_Name`")
        sports = [{'game': i['Sports_Name'], 'url':i['URL'], 'isEnabled':bool(
            i['Enabled']), 'info':i['Info'].split('\\n')} for i in cursor.fetchall()]
        return make_response({'message': sports}), 200
    else:
        cursor.execute(
            "select `Sports_Name`,`URL`,`Info` from `games` where `Enabled`='1' order by `Sports_Name`")
        sports = [{'game': i['Sports_Name'], 'url':i['URL'],
                   'info':i['Info'].split('\\n')} for i in cursor.fetchall()]
        return make_response({'message': sports}), 200
...
def slots():
    ...
    if request.headers['admin-header'].title() == 'Yes':
        ...
        a = cursor.fetchall()
        d = {i: {} for i in ['Monday', 'Tuesday', 'Wednesday',
                             'Thursday', 'Friday', 'Saturday', 'Sunday']}
        isEnabled = {i: False for i in [
        ...
        if request.args.get('pos') == '1':
            cursor.execute(
                f"select `Max_Days` from `games` where `Sports_Name`='{game}'")
            ...
            a = cursor.fetchall()
            cursor.execute(
                f"select `Slot`,`Day` from `team_training` where `Game`='{request.args.get('game').title()}'")
...
def book():
    ...
    if len(check_all) == 0:
        ...
        for i in x['Bookings']:
            cnt2 = 0
            booking_id = ''.join(secrets.choice(
                string.ascii_uppercase + string.digits) for i in range(9))
            cursor.execute(
                f"select exists(select * from `bookings` where `Booking_ID`='{booking_id}') as 'booking_id'")
            if bool(cursor.fetchone()['booking_id']):
                booking_id = ''.join(secrets.choice(
            ...
            for j in x['Bookings'][i]:
                ...
                try:
                    if cnt2 == 0:
                        cursor.execute(
                            f"update `{sports_name}` set `{day}`=`{day}`-1 where `Slots`='{slot}'")
...
def get_bookings(snu_id, booking_id):
    ...
    if booking_id:
        ...
        a = cursor.fetchall()
        b = {'game': a[0]['Game'], 'date': str(a[0]['Date'].strftime(
            '%d/%m/%Y')), 'slot': a[0]['Slot'], 'name': []}
        for i in a:
            if i['Confirm']:
            ...
            if i['SNU_ID'][:len(snu_id)] == snu_id:
                b['confirm'] = i['Confirm']
        cursor.execute(
            f"select `Student_Name` from `bookings` where `Booking_ID`='{booking_id}' limit 1")
        a = cursor.fetchone()
...
def confirm():
    ...
    cursor = mysql.connection.cursor(cur.DictCursor)
    cursor.execute(
        f"select * from bookings where `SNU_ID`='{snu_id}' and `Booking_ID`='{booking_id}'")
    det = cursor.fetchone()
    ...
    cursor.execute(
        f"select exists(select * from `bookings` where `SNU_ID`='{snu_id}' and `Date`='{det['Date']}' and `Cancelled`='0' and `Booking_ID`!='{booking_id}') as duplicate")
    duplicate = bool(cursor.fetchone()['duplicate'])
    cursor.execute(
        f"select exists(select * from `blacklist` where `SNU_ID`='{snu_id}') as blacklist")
    ...
    if not duplicate and not blacklist:
        try:
            ...
            mysql.connection.commit()
            cursor.execute(
                f"update bookings set `Confirm`='1' where `SNU_ID`='{snu_id}' and `Booking_ID`='{booking_id}'")
            mysql.connection.commit()
...
def pending(snu_id):
    ...
    for i in l1:
        cursor.execute(
            f"select `Student_Name` from `bookings` where `Booking_ID`='{i['Booking_ID']}' limit 1")
        a = cursor.fetchone()
...
def admin_bookings(game, date, slot):
    if request.headers['admin-header'].title() == 'Yes':
        ...
        if len(bookings) == 0:
            cursor.execute(
                f"select `{day}` from `{game.title().replace(' ','_')}` where `Slots`='{slot}'")
...
def stop():
    ...
    if request.headers['admin-header'].title() == 'Yes':
        if x['category'] == 'game':
            ...
            date = datetime.now().strftime('%Y-%m-%d')
            cursor.execute(
                f"update `games` set `Enabled`='0' where `Sports_Name`='{x['game'].title()}'")
            mysql.connection.commit()
            cursor.execute(
                f"update `bookings` set `Cancelled`=1, `Cancellation_Date`='{today}', `Confirm`=0 where `Game`='{game}' and `Date`>='{date}'")
            mysql.connection.commit()
            cursor.execute(
                f"update `{game.title().replace(' ','_')}` set `Monday`='0',`Tuesday`='0',`Wednesday`='0',`Thursday`='0',`Friday`='0',`Saturday`='0',`Sunday`='0'")
            mysql.connection.commit()
            ...
            mysql.connection.commit()
            cursor.execute(
                f"update `{game}` set `{day}`='0' where `Slots`='Enabled'")
            mysql.connection.commit()
            ...
            date = datetime.strptime(
                x['date'], '%d-%m-%Y').strftime('%Y-%m-%d')
            cursor.execute(
                f"update `{game}` set `{day}`='0' where `Slots`='{slot}'")
            mysql.connection.commit()
            cursor.execute(
                f"update `bookings` set `Cancelled`=1, `Cancellation_Date`='{today}', `Confirm`=0 where `Game`='{game}' and `Date`='{date}' and `Slot`='{slot}'")
            mysql.connection.commit()
...
def unstop():
    ...
    if request.headers['admin-header'].title() == 'Yes':
        if x['category'] == 'game':
            game = x['game'].title().replace(' ', '_')
            cursor.execute(
                f"update `games` set `Enabled`='1' where `Sports_Name`='{x['game'].title()}'")
            mysql.connection.commit()
            cursor.execute(
                f"select `Capacity` from `games` where `Sports_Name`='{x['game'].title()}'")
            a = cursor.fetchone()['Capacity']
            cursor.execute(
                f"update `{game.title().replace(' ','_')}` set `Monday`='{a}',`Tuesday`='{a}',`Wednesday`='{a}',`Thursday`='{a}',`Friday`='{a}',`Saturday`='{a}',`Sunday`='{a}'")
            mysql.connection.commit()
            cursor.execute(
                f"update `{game.title().replace(' ','_')}` set `Monday`='1',`Tuesday`='1',`Wednesday`='1',`Thursday`='1',`Friday`='1',`Saturday`='1',`Sunday`='1' where `Slots`='Enabled'")
            mysql.connection.commit()
            ...
            day = datetime.strptime(x['date'], '%d-%m-%Y').strftime('%A')
            cursor.execute(
                f"select `Capacity` from `games` where `Sports_Name`='{game}'")
            a = cursor.fetchone()['Capacity']
            cursor.execute(f"update `{game}` set `{day}`='{a}'")
            mysql.connection.commit()
            cursor.execute(
                f"update `{game}` set `{day}`='1' where `Slots`='Enabled'")
            mysql.connection.commit()
            ...
            a = cursor.fetchone()['Capacity']
            cursor.execute(
                f"update `{game}` set `{day}`='{a}' where `Slots`='{slot}'")
            mysql.connection.commit()
...
def booking_count():
    ...
    if request.args.get('category').lower() == 'game':
        ...
        date = datetime.now().strftime('%Y-%m-%d')
        cursor.execute(
            f"select count(`SNU_ID`) as Num from `bookings` where `Date`>='{date}' and `Game`='{game}' and `Confirm`=1")
        return make_response({'message': str(cursor.fetchone()['Num'])})
    elif request.args.get('category').lower() == 'date':
        game = request.args.get('game').title().replace(' ', '_')
        date = datetime.strptime(request.args.get(
            'date'), '%d-%m-%Y').strftime('%Y-%m-%d')
        cursor.execute(
            f"select count(`SNU_ID`) as Num from `bookings` where `Game`='{game}' and `Date`='{date}'")
        return make_response({'message': str(cursor.fetchone()['Num'])})
        ...
        cursor.execute(
            f"select count(`SNU_ID`) as Num from `bookings` where `Game`='{game}' and `Date`='{date}' and `Slot`='{slot}'")
        return make_response({'message': str(cursor.fetchone()['Num'])})
...
def present():
    ...
    x = request.get_json()
    cursor.execute(
        f"update `bookings` set `Present`='1' where `SNU_ID`='{x['snu_id']}' and `Booking_ID`='{x['booking_id']}'")
    mysql.connection.commit()
    cursor.execute(
        f"select exists(select * from blacklist where `SNU_ID`='{x['snu_id']}' and `Booking_ID`='{x['booking_id']}') as bool")
    if cursor.fetone()['bool']:
        cursor.execute(
            f"delete from `blacklist` where `SNU_ID`='{x['snu_id']}'")
...
def present():
    ...
    x = request.get_json()
    cursor.execute(
        f"update `bookings` set `Present`='1' where `Booking_ID`='{x['booking_id']}'")
    mysql.connection.commit()
    cursor.execute(
        f"select exists(select * from blacklist where `Booking_ID`='{x['booking_id']}') as bool")
    if cursor.fetone()['bool']:
        cursor.execute(
            f"delete from `blacklist` where `Booking_ID`='{x['booking_id']}'")
        mysql.connection.commit()
    return make_response({'message': f"{x['name']} has been marked present"})
...
def absent():
    ...
    x = request.get_json()
    cursor.execute(
        f"update `bookings` set `Present`='0' where `SNU_ID`='{x['snu_id']}' and `Booking_ID`='{x['booking_id']}'")
    mysql.connection.commit()
    com = "INSERT INTO `blacklist`(`Student_Name`, `SNU_ID`, `Date`, `Booking_ID`) VALUES (%s,%s,%s,%s)"
    val = (str(x['name']), str(x['snu_id']), str(
        datetime.now().strftime('%Y-%m-%d %H:%M:%S')), str(x['booking_id']))
    cursor.execute(com, val)
    mysql.connection.commit()
...
def slot_capacity_change():
    ...
    if request.headers['admin-header'].title() == 'Yes':
        if request.method == 'GET':
            game = request.args.get('game').title().replace(' ', '_')
            day = datetime.strptime(request.args.get(
                'date', type=str), '%d-%m-%Y').strftime('%A')
            slot = request.args.get('slot')
            cursor.execute(
                f"select `{day}` as 'Capacity' from `{game}` where `Slots`='{slot}'")
            ...
            date = datetime.strptime(
                x['date'], '%d-%m-%Y').strftime('%Y-%m-%d')
            cursor.execute(
                f"select `Capacity` from `games` where `Sports_Name`='{x['game']}'")
            capacity = int(x['capacity'])
            slot = x['slot']
            cap = int(cursor.fetchone()['Capacity'])
            cursor.execute(
                f"select count(*) as num from `bookings` where `Game`='{game}' and `Date`='{date}' and `Slot`='{slot}' and `Confirm`='1'")
            bookingNum = cursor.fetchone()['num']
            if cap-bookingNum >= capacity:
                cursor.execute(
                    f"update `{game}` set `{day}`='{capacity}' where `Slots`='{slot}'")
                mysql.connection.commit()
...
def team_training():
    ...
    if request.method == 'POST':
        ...
        if request.args.get('option').lower == 'on':
            ...
            cursor.execute(
                f"delete from `team_training` where `Game`='{x['game']}' and `Slot`='{x['slot']}' and `Day`='{x['day']}'")
            mysql.connection.commit()
...
def download_data():
    ...
    if request.headers['admin-header'].title() == 'Yes':
        ...
        if datetime.now()-timedelta(weeks=52) <= datetime.strptime(x['start'], '%d-%m-%Y'):
            cursor.execute(
                f"select * from `bookings` where `Date`>='{start}' and `Date`<'{end}'")
